...
<div class="d-flex justify-content-end">
    ...
    <a href="{{ route('admin.camp.create') }}" class="btn btn-success m-1">
...
<div class="">
    ...
    <div class="form-check form-switch">
        <input name="is_active" id="is_active" data-id="{{$camp->id}}" value="true" data-bs-toggle="toggle"  {{ isset($camp) && $camp->is_active == 'true' ? 'checked' : '' }} class="form-check-input" type="checkbox" role="switch" />
    </div>
</td>
						<td>
                    <a href="{{ route('admin.camp.show', ['id' => $camp->id]) }}" class="btn btn-primary btn-sm">
                        <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.camp.edit', ['id' => $camp->id]) }}" class="btn btn-success btn-sm">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                    <a href="#" data-id="{{ $camp->id }}" class="btn btn-danger btn-sm deleteBtn">
        ...
        <div class="d-flex justify-content-center">
            {{ $camps->links('pagination::bootstrap-5') }}
        </div>
    </div>
</div>
</div>
...
@section('scripts')
    ...
    <script>
        ...
        checkbox.onchange = async (event) => {
            ...
            const response = await fetch('/admin/camps/speed/' + id, {
                method: 'PUT',
        ...
        deleteButtons.forEach(deleteButton => {
            deleteButton.addEventListener('click', (event)=>{
                ...
                confirmDeleteBtn.addEventListener('click',async ()=>{
                    const csrfToken = document.head.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('/admin/camps/delete/'+id , {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const tableHeaders = document.querySelectorAll('#Camp th');
            ...
            tableHeaders.forEach(function(header, index) {
                ...
                const savedSelection = localStorage.getItem('selectedColumns#Camp');
        ...
        function toggleColumn(columnIndex, show) {
            const dataTable = document.getElementById('Camp');
        ...
        function saveSelection() {
            ...
            localStorage.setItem('selectedColumns#Camp', JSON.stringify(selectedColumns));
        }
        ...
        function sortTable(columnIndex) {
            const table = document.getElementById('Camp');
